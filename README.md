...
- 2.0.0: new builtin spinners, bars and themes, with some renamed to make them more widely usable; spinners engine revamp, with standardized factory signatures, improved performance and new features: smoother bouncing spinners with an additional frame at the edges, new animation mode in compound spinners which can now run alongside or in sequence, nicer effect in alongside compound spinners using weighted spreading over the available space, smoother animation in scrolling spinners when the input is longer than available space; showtime now has themes! and is more concise, does not wrap screen and can filter patterns; `bar()` handle now supports absolute and relative positioning in all modes; improved logging for files, enriched as print; new advanced configuration for `unknown` bar, supporting spinner names, spinner factories and unknown bar factories; uses `time.perf_counter()` high resolution clock; improved `print_chars()` utility; requires python 3.6+ (and officially supports python 3.9 and 3.10)
