...
- 2.0.0: showtime now has themes! it is also more concise, does not wrap screen and can filter patterns; `bar()` handle now supports absolute and relative positioning, in both definite and manual modes; improved logging for files, enriched as print; new advanced configuration for `unknown` bar, supporting spinner names, spinner factories and unknown bar factories; uses `time.perf_counter()` high resolution clock; improved `print_chars()` utility; requires python 3.6+ (and officially supports python 3.9 and 3.10)
