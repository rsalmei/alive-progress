...
- 2.0.0: `bar()` handle now supports absolute and relative positioning, in both definite and manual modes; improved logging for files, enriched as print; uses `time.perf_counter()` high resolution clock; improved `print_chars()` utility; requires python 3.6+ (and officially supports python 3.9 and 3.10)
