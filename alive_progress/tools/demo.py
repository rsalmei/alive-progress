...
cases += [
    Case(name.capitalize(), 1000, {**features[i % len(BARS)], **config}, done=True)
...
if __name__ == "__main__":
    parser, run = toolkit(
        "Demonstrates alive-progress, showcasing several common scenarios."
    )
    parser.add_argument(
        "sleep", type=float, nargs="?", help="the sleep time (default=.003)"
    )
