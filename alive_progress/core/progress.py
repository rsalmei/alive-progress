...
@contextmanager
def __alive_bar(
    config, total=None, *, calibrate=None, _cond=threading.Condition, _sampling=False
):
    ...
    def run(spinner_player, spinner_suffix):
        with cond_refresh:
            while thread:
                ...
                cond_refresh.wait(1.0 / fps(run.rate))
    ...
    def set_text(text=None):
        if text and config.dual_line:
            run.text, run.suffix = (
                "\n",
                to_cells(str(text)),
            ), term.cursor_up_1.sequence
        else:
            run.text, run.suffix = (
                to_cells(None if text is None else str(text)),
            ), ""  # 1-tuple.
    ...
    if config.manual:
        ...
        def bar_handle(percent):  # for manual progress modes.
            ...
            run.percent = max(0.0, float(percent))
            update_hook()
    ...
    if total or not config.manual:  # we can count items.
        ...
        rate_spec, factor, header = "f", 1.0e6, "on {:d}: "
    else:  # there's only a manual percentage.
        logic_total, current = 1.0, lambda: run.percent
        rate_spec, factor, header = "%", 1.0, "on {:.1%}: "
    ...
    run.last_len, run.elapsed, run.count, run.percent = 0, 0.0, 0, 0.0
    run.rate, run.init, run.text, run.title, run.suffix = 0.0, 0.0, None, None, None
    ...
    monitor_end = _Widget(
        monitor_end, config.monitor_end, monitor.f[:-1]
    )  # space separator.
    elapsed = _Widget(elapsed_run, config.elapsed, elapsed_default)
    elapsed_end = _Widget(
        elapsed_end, config.elapsed_end, elapsed.f[:-1]
    )  # space separator.
    stats = _Widget(stats_run, config.stats, stats_default)
    stats_end = _Widget(
        stats_end, config.stats_end, stats_end_default if stats.f[:-1] else ""
    )

    ctrl_c, bar = False, __AliveBarHandle(
        pause_monitoring, current, set_title, set_text
    )
    set_text(), set_title()
    start_monitoring()
    try:
...
def alive_it(it, total=None, *, finalize=None, calibrate=None, **options):
    ...
    return __AliveBarIteratorAdapter(
        it, finalize, __alive_bar(config, total, calibrate=calibrate)
    )
...
class __AliveBarIteratorAdapter:
    ...
    def __call__(self, *args, **kwargs):
        raise UserWarning(
            "The bar position is controlled automatically with `alive_it`."
        )
