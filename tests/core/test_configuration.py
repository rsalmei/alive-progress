...
from alive_progress.core.configuration import (
    ERROR,
    Config,
...
from alive_progress.utils.terminal import FULL, NON_TTY


@pytest.mark.parametrize(
    "lower, upper, num, expected",
    [
        (100, 110, 100, 100),
        (100, 110, 110, 110),
        (100, 110, -1, ERROR),
...
def test_style_input_factory(param, expected):
    ...
    func = __style_input_factory(
        NAMES,
        test_style_input_factory,
        "artifact_super_cool_compiler_assembler_factory",
        None,
    )
    assert func(param) == expected
...
def test_style_input_factory_error(param):
    test_style_input_factory_error.__file__ = (
        ""  # simulates a func_style declared elsewhere.
    )

    func = __style_input_factory(
        NAMES,
        test_style_input_factory_error,
        "artifact_super_cool_compiler_assembler_factory",
        None,
    )
...
@pytest.fixture(
    params=[
        ...
        (
            dict(force_tty=True, manual=True, enrich_print=False, title_length=10),
            dict(force_tty=FULL),
        ),
