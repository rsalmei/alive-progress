...
@pytest.mark.parametrize(
    "length, block, background, right, hide, expected",
    [
...
def test_scrolling_spinner(length, block, background, right, hide, expected):
    spinner_factory = scrolling_spinner_factory(
        "abc", length, block, background, right=right, hide=hide
    )
    spinner = spinner_factory()  # natural spinner size.
    assert tuple(tuple(join_cells(f) for f in spinner()) for _ in expected) == expected
...
@pytest.mark.parametrize(
    "length, block, background, hide, expected",
    [
        (
            None,
            None,
...
def test_bouncing_spinner(length, block, background, hide, expected):
    spinner_factory = bouncing_spinner_factory(
        ("abc", "def"), length, block, background, right=True, hide=hide
    )
    spinner = spinner_factory()  # natural spinner size.
    assert tuple(tuple(join_cells(f) for f in spinner()) for _ in expected) == expected
...
def test_alongside_spinner_with_pivot(inputs, expected, spinner_test):
    spinner_factory = alongside_spinner_factory(
        *(spinner_test(x) for x in inputs), pivot=0
    )
    spinner = spinner_factory()  # natural spinner size.
    assert tuple(tuple(join_cells(f) for f in spinner()) for _ in expected) == expected
...
def test_sequential_spinner_no_intermix(inputs, expected, spinner_test):
    spinner_factory = sequential_spinner_factory(
        *(spinner_test(*x) for x in inputs), intermix=False
    )
    spinner = spinner_factory()  # natural spinner size.
    assert tuple(tuple(join_cells(f) for f in spinner()) for _ in expected) == expected
