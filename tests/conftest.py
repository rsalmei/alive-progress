...
def spinner_test():
    def spinner_test(*data):
        def inner_factory(actual_length=None):
            ...
            data = tuple(
                tuple(tuple(d * ratio)[:actual_length] for d in cycle) for cycle in data
            )

            # generate spec info, just like the real one.
            frames = tuple(len(cycle) for cycle in data)
            inner_spinner.__dict__.update(
                cycles=len(data),
                length=len(data[0][0]),
                frames=frames,
        ...
        data = tuple(
            tuple(cycle) if isinstance(cycle, str) else cycle for cycle in data
        )
        # simulate to_cells().
